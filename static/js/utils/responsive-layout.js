...
class ResponsiveLayout {
    ...
    detectBreakpoint() {
        // ✅ Toujours utiliser window.innerWidth pour détecter le breakpoint.
        // Utiliser canvas.offsetWidth était incorrect : le canvas peut être
        // plus étroit que 768px sur PC (sidebar qui réduit l'espace, max-width CSS...)
        // ce qui déclenchait le mode mobile sur desktop.
    ...
    getAvailableWidth() {
        ...
        if (bp === 'mobile') {
            // Sur mobile, la sidebar est cachée. On utilise la largeur d'écran
            // moins le padding. Le canvas.offsetWidth peut être lu aussi mais
            // window.innerWidth est plus fiable dès l'init.
            const canvas = document.querySelector('.canvas-container');
            const canvasWidth = canvas ? canvas.offsetWidth : screenWidth;
            const usable = Math.min(canvasWidth, screenWidth - (this.config.contentPadding * 2));
            console.log(`📱 Mobile: disponible = ${usable}px (écran ${screenWidth}px)`);
            return usable;
        }
        ...
        // Cap à editorCanvasWidth : si l'écran est plus grand, le canvas ne grandit pas.
    ...
    getScalingRatio(availableWidth) {
        ...
        // Sur desktop/wide : si le ratio >= 1, on ne fait rien (les composants
        // restent à leurs coordonnées d'origine). C'est le CSS qui gère.
        // Sur mobile/tablet : on calcule le vrai ratio depuis la largeur disponible
        // sans cap artificiel (l'ancien cap 0.6/0.8 causait un mauvais scaling).
        // Minimum absolu à 0.15 pour les écrans très étroits.
        if (bp === 'mobile' || bp === 'tablet') {
            return Math.max(ratio, 0.15);
        }

        // Desktop/wide : retourner 1.0 au maximum pour ne jamais agrandir
    ...
    applyResponsive(components, availableWidth) {
        ...
        console.log(`📐 Ratio: ${(ratio * 100).toFixed(1)}% | BP: ${bp}`);
        
        // ✅ Sur desktop/wide avec ratio = 1.0 : ne rien modifier.
        // Les composants restent exactement à leurs coordonnées de l'éditeur.
        // Appliquer le responsive sur desktop causait des chevauchements
        // quand le canvas éditeur avait une largeur différente de editorCanvasWidth.
        if (ratio >= 1.0 && (bp === 'desktop' || bp === 'wide')) {
            console.log('✅ Desktop/wide — aucun scaling appliqué');
            return this.deepClone(components);
        }
    ...
    setupResizeListener() {
        let timer;
        let lastWidth = this.getAvailableWidth();
        ...
        window.addEventListener('resize', () => {
            ...
            timer = setTimeout(() => {
                ...
                // Recalculer si le breakpoint OU la largeur a changé (rotation écran)
                if (newBp !== this.currentBreakpoint || widthChanged) {
                    console.log(`🔄 ${this.currentBreakpoint} → ${newBp} | ${lastWidth}px → ${newWidth}px`);
